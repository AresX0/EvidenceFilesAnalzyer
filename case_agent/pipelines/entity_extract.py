...
from ..db.models import EvidenceFile, Entity
...
def extract_entities_for_file(path: Path, db_path=None):
    ...
    if not file_row:
        ...
        return []
    # Gather text for this file
    texts = session.query(EvidenceFile).filter_by(id=file_row.id)
    # Prefer ExtractedText entries
    ...
    for block in blocks:
        text = block.text or ""
        provenance = {"sha256": file_row.sha256, "path": file_row.path, "page": block.page}
        seen = set()
        if nlp:
            try:
                max_len = getattr(nlp, 'max_length', 1000000)
                if len(text) > max_len:
                    logger.info("Text length %d exceeds spaCy max_length %d; chunking", len(text), max_len)
                    ...
                    while offset < len(text):
                        ...
                        try:
                            ...
                            logger.exception("spaCy chunk NER failed at offset %d for %s: %s", offset, path, e)
                            # Fallback to regex heuristics for this chunk
                            for m in DATE_RE.finditer(chunk):
                                key = ("DATE", m.group(0), block.page, m.start() + offset)
                                if key in seen:
                                    continue
                                seen.add(key)
                                e = Entity(file_id=file_row.id, entity_type="DATE", text=m.group(0), span=f"{m.start() + offset}-{m.end() + offset}", provenance=provenance, confidence="medium")
                                session.add(e)
                                entities.append({"entity_type": e.entity_type, "text": e.text, "confidence": e.confidence, "provenance": e.provenance})

                            for m in re.finditer(r"\b([A-Z][A-Za-z0-9&'\-\.\s]{2,}?(?:\s+(?:Inc|Ltd|LLC|Corp|Co|Company|Corporation)))\b",
                                                chunk):
                                org = m.group(1).strip()
                                ...
                                if key in seen:
                                    continue
                                seen.add(key)
                                e = Entity(file_id=file_row.id, entity_type="ORG", text=org, span=f"{m.start() + offset}-{m.end() + offset}", provenance=provenance, confidence="medium")
                                session.add(e)
                                entities.append({"entity_type": e.entity_type, "text": e.text, "confidence": e.confidence, "provenance": e.provenance})

                            for m in re.finditer(r"\b([A-Z][a-z]{2,}\s+[A-Z][a-z]{2,})\b", chunk):
                                name = m.group(1)
                                key = ("PERSON", name, block.page, m.start() + offset)
                                if key in seen:
                                    continue
                                seen.add(key)
                                e = Entity(file_id=file_row.id, entity_type="PERSON", text=name, span=f"{m.start() + offset}-{m.end() + offset}", provenance=provenance, confidence="low")
                                session.add(e)
                                entities.append({"entity_type": e.entity_type, "text": e.text, "confidence": e.confidence, "provenance": e.provenance})
                        else:
                            for ent in doc.ents:
                                if ent.label_ in {"PERSON", "ORG", "DATE", "TIME", "GPE"}:
                                    key = (ent.label_, ent.text, block.page, ent.start_char + offset)
                                    if key in seen:
                                        continue
                                    seen.add(key)
                                    e = Entity(file_id=file_row.id, entity_type=ent.label_, text=ent.text.strip(), span=f"{ent.start_char + offset}-{ent.end_char + offset}", provenance=provenance, confidence="high")
                                    session.add(e)
                                    entities.append({"entity_type": e.entity_type, "text": e.text, "confidence": e.confidence, "provenance": e.provenance})
                        offset += chunk_size
