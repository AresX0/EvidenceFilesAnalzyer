...
def _align_face(pil_img, landmarks):
    ...
    left = int((w - side) // 2)
    top = int((h - side) // 2)
    crop = rotated.crop((left, top, left + side, top + side)).resize((160, 160), Image.LANCZOS)
    return crop
...
def _compute_embedding(image_input):
    ...
    if face_recognition is not None:
        try:
            ...
            face_locs = face_recognition.face_locations(np_img) if np_img is not None else []
            if face_locs:
                # pick largest face bbox
                loc = max(face_locs, key=lambda l: (l[2]-l[0])*(l[1]-l[3] if l[1]>l[3] else l[3]-l[1]))
                # landmarks
        ...
        try:
            ...
            if face_recognition is not None:
                try:
                    ...
                    if locs:
                        l = max(locs, key=lambda r: (r[2]-r[0])*(r[1]-r[3] if r[1]>r[3] else r[3]-r[1]))
                        lds = face_recognition.face_landmarks(np_tmp, [l])
                        if lds:
...
def find_faces_in_image(image_path: Path):
    ...
    if face_recognition is not None:
        ...
        for loc, enc in zip(locations, encs):
            ...
            out.append({"bbox": {"top": int(loc[0]), "right": int(loc[1]), "bottom": int(loc[2]), "left": int(loc[3])}, "embedding": enc.tolist()})
        return out
    ...
    if cv2 is not None:
        try:
            ...
            for (x, y, w, h) in rects:
                ...
                if emb is not None:
                    out.append({"bbox": {"top": top, "right": right, "bottom": bottom, "left": left}, "embedding": emb.tolist() if hasattr(emb, 'tolist') else list(emb)})
            return out
        except Exception:
...
def find_faces_in_video(video_path: Path, interval_seconds: float = 5.0):
    ...
    if cv2 is None:
        logger.warning('opencv (cv2) not installed; cannot sample video frames for %s', video_path)
        return []
    ...
    while ts < duration:
        ...
        dets = []
        for loc, enc in zip(locations, encs):
            dets.append({"bbox": {"top": int(loc[0]), "right": int(loc[1]), "bottom": int(loc[2]), "left": int(loc[3])}, "embedding": enc.tolist()})
        if dets:
...
def search_gallery_for_image(image_path: Path, gallery_dir: Path, threshold: float = 0.6, top_k: int = 5):
    ...
    if not faces:
        logger.info('No faces detected (or detector unavailable) for %s; using whole-image embedding as a probe', image_path)
        ...
        return {'source': str(image_path), 'num_faces': 1, 'results': [{'face_bbox': None, 'matches': best[:top_k]}]}
...
def search_gallery_for_video(video_path: Path, gallery_dir: Path, interval_seconds: float = 5.0, threshold: float = 0.6, top_k: int = 3):
    gallery_dir = Path(gallery_dir)
    gallery_embs = _load_gallery_embeddings(gallery_dir)
...
def _compute_subject_embeddings(labeled: dict) -> dict:
    ...
    for subject, items in labeled.items():
        embs = [(_np.array(it['embedding'], dtype='float32')) for it in items if it.get('embedding') is not None]
        if not embs:
            continue
...
def search_labeled_gallery_for_image(image_path: Path, labeled_gallery_dir: Path, threshold: float = 0.6, top_k: int = 5, use_subject_embeddings: bool = True):
    ...
    if use_subject_embeddings:
        ...
        for subject, s_emb in subj_embs.items():
            ...
            if dist <= threshold:
                subject_matches.append({'subject': subject, 'best_distance': float(dist), 'matches': []})
        ...
        if subject_matches:
            ...
            return {'source': str(image_path), 'num_subjects': len(subject_matches), 'subject_matches': subject_matches}
    ...
    for subject, items in labeled.items():
        ...
        if best:
            best.sort(key=lambda x: x['distance'])
            subject_matches.append({'subject': subject, 'best_distance': best[0]['distance'], 'matches': best[:top_k]})
    subject_matches.sort(key=lambda x: x['best_distance'])
    return {'source': str(image_path), 'num_subjects': len(subject_matches), 'subject_matches': subject_matches}
    ...
    for sm in res.get('subject_matches', []):
        ...
        for m in sm.get('matches', []):
            if best is None or m.get('distance') < best.get('distance'):
                best = m
        if best is not None:
            out.append({'subject': subject, 'best_distance': best.get('distance'), 'best_path': best.get('path')})
    return out
...
def _persist_results(db_path: str | Path, res: dict, aggregate: bool = False):
    ...
    if 'subject_matches' in res:
        if aggregate:
            ...
            for s in summary:
                fm = FaceMatch(source=source, probe_bbox=None, subject=s.get('subject'), gallery_path=s.get('best_path'), distance=float(s.get('best_distance')), created_at=now)
                session.add(fm)
        else:
            for sm in res.get('subject_matches', []):
                subject = sm.get('subject')
                for m in sm.get('matches', []):
                    fm = FaceMatch(source=source, probe_bbox=None, subject=subject, gallery_path=m.get('path'), distance=float(m.get('distance')), created_at=now)
                    session.add(fm)
    else:
...
def cli_run(args):
    ...
    if getattr(args, 'labeled', False):
        if p.suffix.lower() in {'.jpg', '.jpeg', '.png'}:
            res = search_labeled_gallery_for_image(p, gallery, threshold=args.threshold, top_k=args.top_k)
        else:
            raise SystemExit('Labeled search only supports images')
    else:
        if p.suffix.lower() in {'.jpg', '.jpeg', '.png'}:
            res = search_gallery_for_image(p, gallery, threshold=args.threshold, top_k=args.top_k)
        else:
            res = search_gallery_for_video(p, gallery, interval_seconds=args.interval, threshold=args.threshold, top_k=args.top_k)
